...
| Relay proxy | The cloud service between the device and Azure: it terminates the provider-neutral device WebSocket, adapts Protocol v0 audio to the configured Realtime capability, owns conversation state, and hosts the optional governed delegation boundary while calling Azure AI Foundry keylessly | [ADR-0002](adr/ADR-0002-overall-architecture.md) + [ADR-0004](adr/ADR-0004-audio-pipeline.md) |
| Realtime path | The direct routine-conversation path from Protocol v0 through the relay audio adapter to the swappable `stackchan-realtime` speech-to-speech capability and back, with Azure details hidden from the device | [ADR-0004](adr/ADR-0004-audio-pipeline.md) |
| Governed intelligence path | The exceptional path that turns a complex or tool-requiring request into one bounded MAF task whose Responses-capability and tool actions pass through AGT before returning a result to the Realtime session | [ADR-0004](adr/ADR-0004-audio-pipeline.md) |
| Microsoft Agent Framework (MAF) | The application framework used for the initial single, task-scoped delegated agent and its model/tool invocation boundary; workflows and multi-agent orchestration are not implied | [ADR-0004](adr/ADR-0004-audio-pipeline.md) |
| Agent Governance Toolkit (AGT) | The in-process action-policy layer that checks delegated capabilities and tools against fail-closed, deny-by-default rules; it does not validate model reasoning, factual truth, or physical safety | [ADR-0004](adr/ADR-0004-audio-pipeline.md) |
